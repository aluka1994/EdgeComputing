...
public static void uploadFileList(ArrayList<File> file_paths, String bucket_name,
        String key_prefix, boolean pause,TransferManager xfer_mgr)
{
...
public static void uploadFile(File file_path, String bucket_name,
        String key_prefix, boolean pause,TransferManager xfer_mgr)
{
...
//    }
    ...
    try {
        Upload xfer = xfer_mgr.upload(bucket_name, key_prefix, file_path);
        // loop with Transfer.isDone()
        XferMgrProgress.showTransferProgress(xfer);
        //  or block with Transfer.waitForCompletion()
        XferMgrProgress.waitForCompletion(xfer);
...
public static void main(String[] args)
{
    ...
    System.out.println( dirs_to_copy.size() > 0 ? "XferMgrUpload: time taken for dirs copy "+ (t1 - t0) : "No dir to copy");
    ...
    if (files_to_copy.size() > 1) {
        ...
        System.out.println("XferMgrUpload: time taken to copy multiple files " + (t3 - t2));
    } else if (files_to_copy.size() == 1) {
  	  long t4 = System.currentTimeMillis();
  	  	
        //uploadFile(files_to_copy.get(0), bucket_name, key_prefix, pause,s3);
        long t5 = System.currentTimeMillis();
        System.out.println("XferMgrUpload: time taken to copy single file " + (t5 - t4));
    } // else: nothing to do.
}
}